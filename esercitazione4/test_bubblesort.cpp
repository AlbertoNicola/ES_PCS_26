...
int main(void)
{
	...
	for (size_t i=0; i < 100; i++) {
		...
		if (!is_sorted(vi))
		{
			std::cout << "Test Fallito: il valore " << i << " non è stato sistemato (bubblesort). \n";
			return EXIT_FAILURE;
		}
	}
	...
	if (!is_sorted(vs))
	{
		std::cout << "Test Fallito: la stringa non è stato sistemato (bubblesort) \n";
		return EXIT_FAILURE;
	}
	
	std::cout << "BubbleSort: Test andato a buon fine \n";
	return EXIT_SUCCESS;
}